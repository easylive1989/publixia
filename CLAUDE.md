...
首頁只呈現台股：區間 tabs、半年 dropdown、所選日量能判讀、法人明細、指數／法人
組合圖，以及自算情緒指數／加權指數雙軸折線圖。冷熱逐日明細與完整計算原理集中在
`/details`，由主冷熱圖卡內的明確入口前往；舊 `/method` 必須轉址以保留書籤。點選組合圖任一交易日，所選日量能與法人明細必須同步切換；
